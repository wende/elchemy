...
compile-std-tests-watch:
	( find elchemy-core \( -name "*.elm" -or -name '*.ex*' \) | grep -v "elchemy.ex" | grep -v ".#" | grep -v "elm-stuff" ; find src -name "*.elm" ) | entr bash -c "make compile && make compile-std && make test-std"
